...
# ****************************************************************************
# This script is downloaded from https://aka.ms/InstallAzureCLIDeb
# And modified to use `--insecure` on curl commands as a workaroun
# to SSL cert verification issues in Docker builds on DWP MacBooks
# ****************************************************************************
...
set -v
curl -sSL --insecure https://packages.microsoft.com/keys/microsoft.asc | gpg --dearmor > /etc/apt/trusted.gpg.d/microsoft.asc.gpg
set +v
